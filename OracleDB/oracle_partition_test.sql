...
Oracle Partition 测试总结

1、建表的时候，需要建立partiton的范围。例如按月，或按日进行分区。
2、分区需要确定第一个分区，后续的分区可以使用自动分区进行。如果分区字段使用默认值，那么只能是数据写入的时候，该字段不写入任何值的时候default才能生效。
3、如果分区不存在，分区会自动根据数据插入时候的分区字段建立分区；如果存在，则直接将数据写入分区中。
4、如果写入的数据时间在第一个分区之前，默认写入第一个分区，而不是根据数据创建一个新分区。
